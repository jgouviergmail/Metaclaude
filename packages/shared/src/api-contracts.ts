 ...
 *
 * Two states, both explicit. `global` means every workspace, including any
 * created later — the semantics the old nullable `workspaceId` column carried,
 * kept because losing it would make every new workspace start bare. Otherwise
 * the reach is `workspaceIds`, which may be empty: an extension attached to
 * nothing exists in the library and is mounted nowhere, a state the single
 * column could not express at all.
 *
 * `workspaceId` stays on the records beside this, and is now only a record of
 * where a row was first created. Nothing resolves with it.
 */
export const ExtensionReach = z.object({
  global: z.boolean(),
  workspaceIds: z.array(z.string().min(1).max(64)).max(200),
});
export type ExtensionReach = z.infer<typeof ExtensionReach>;
...
export const SkillDefinition = z.object({
  ...
  category: LibraryCategory.default('general'),
  enabled: z.boolean(),
  /** Every workspace, including future ones. */
  isGlobal: z.boolean().default(false),
