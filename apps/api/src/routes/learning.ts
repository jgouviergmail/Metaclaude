...
export function registerLearningRoutes(app: App, context: AppContext): void {
  ...
  app.post<{ Params: { id: string } }>('/api/insights/:id/install-skill', async (request, reply) => {
    ...
    // Normalised here as well as at drafting, because the proposals already
    // sitting in the review queue were drafted before that existed and their
    // names are what they are.
    const name = toSkillName(proposal.name);
    if (!name) throw new HttpError(422, 'The proposed skill name is unusable.');
