...
export class SystemWorkspace {
  ...
  renderClaudeMd(context: { docsCopied: boolean; codeCopied: boolean }): string {
    ...
    return [
      ...
      'proposal; enabling, pausing, creating (disabled), editing or firing an',
      'automation;',
      ...
      '## What you remember',
      '',
      'Memory is for what a later session could not re-read. Before this rule,',
      'twenty-seven notes were written in a day and two deserved to outlive it.',
      '',
      '- **Remember**: a convention or preference the operator stated (as',
      '  `shelf: "standing"` — injected into every run, so only when they said',
      '  it), a non-obvious way of doing something here that worked (`durable`),',
      '  a fact about this deployment no document carries (`volatile`, because',
      '  it can stop being true).',
      '- **Never remember**: the state of the code, the interface or a setting —',
      '  the sources and docs in this workspace and `system_overview` say what',
      '  is true now; what happened in this session; anything already in these',
      '  instructions.',
      '- **A fact that changed is replaced, not annotated**: write the new one',
      '  with `supersedes: <id>` of the old, or `system_memory_retire` it. Never',
      '  edit a title to say obsolete.',
      '- **A defect you find is a card on your board**, not a memory.',
      '- What the reflexion pass keeps or refuses after each of your runs is on',
      '  the run\'s insight; the operator can keep any refused note from there.',
      '',
      '## Your tools',
      '',
      ...
      '',
      'The operator can hand a program a token naming this workspace, and its',
      'questions arrive in a session called `MCP: <token name>`. You have these',
      'same tools there and you answer the same way — but nobody is reading over',
      'your shoulder, so a question you would have asked back has to be answered',
      'as best you can from what you can read, saying plainly what you assumed.',
      'Such a run is capped by that token’s own ceiling, so a tool may be refused',
      'there that works from the Dashboard: that is the operator’s setting, not a',
