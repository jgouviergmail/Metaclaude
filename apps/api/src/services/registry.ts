...
export class Registry {
  ...
  upsertSkill(input: {
    ...
    if (input.id) {
      ...
      const updated = this.getSkill(input.id);
      if (!updated) throw new RegistryError('Skill not found.', 404);
      return updated;
    }
    ...
    try {
      ...
      if (String(error).includes('UNIQUE')) {
        throw new RegistryError(`A skill named "${input.name}" already exists in this scope.`, 409);
      }
      throw error;
    }
  ...
  upsertAgent(input: {
    ...
    if (input.id) {
      ...
      const updated = this.getAgent(input.id);
      if (!updated) throw new RegistryError('Agent not found.', 404);
      return updated;
    }
    ...
    try {
      ...
      if (String(error).includes('UNIQUE')) {
        throw new RegistryError(`An agent named "${input.name}" already exists in this scope.`, 409);
      }
      throw error;
    }
  ...
  upsertMcpServer(input: {
    ...
    const existingKeys = input.id ? (this.getMcpServer(input.id)?.envKeys ?? []) : [];
    ...
    if (input.id) {
      ...
      try {
        ...
        if (String(error).includes('UNIQUE')) {
          throw new RegistryError(`An MCP server named "${input.name}" already exists.`, 409);
        }
        throw error;
      }
    }
