...
export const fr: Record<string, string> = {
  ...
  '{n} automations are hidden by them.': '{n} automatisations sont masquées par eux.',
  ...
  'More rows at a glance.': "Plus de lignes d'un coup d'œil.",
  'More room, and help always shown.': "Plus d'air, et l'aide toujours affichée.",
  ...
  'Filter by kind': 'Filtrer par type',
  'Something is broken': 'Quelque chose est cassé',
  'Something must be done': 'Quelque chose est à faire',
  'Something could be better': 'Quelque chose pourrait être mieux',
  Archive: 'Archiver',
  Restore: 'Restaurer',
  ...
  'Recompute every vector — memories, documents and the classifier’s examples — with the embedder in force. It runs by itself after a change of provider; press it if the page still says vectors are waiting.':
    'Recalculer chaque vecteur — mémoires, documents et exemples du classificateur — avec l’embedder en vigueur. Cela se fait tout seul après un changement de fournisseur ; appuyez si la page dit encore que des vecteurs attendent.',
  ...
  system: 'système',
  'Runs the same retrieval the agent runs before a prompt. Right now it matches words, not meaning — this is what would actually be injected into context.': 'Lance la même recherche que l’agent avant un prompt. En ce moment elle compare les mots, pas le sens — c’est ce qui serait réellement injecté dans le contexte.',
  'Search memory by words': 'Rechercher dans la mémoire par les mots',
  Retrieval: 'Recherche',
  'Search matches meaning, in French and English alike.': 'La recherche compare le sens, en français comme en anglais.',
  'Model loading': 'Modèle en chargement',
  'Search matches words until the model is ready; new memories wait for their vectors.': 'La recherche compare les mots jusqu’à ce que le modèle soit prêt ; les nouvelles mémoires attendent leurs vecteurs.',
  'Model unavailable': 'Modèle indisponible',
  'The model did not load, so search matches words, not meaning. The doctor says why.': 'Le modèle n’a pas pu se charger : la recherche compare les mots, pas le sens. Le doctor en dit la raison.',
  'Words only': 'Mots seulement',
  'The built-in hashing embedder matches words, not meaning.': 'L’embedder de hachage intégré compare les mots, pas le sens.',
  'The server has not said what retrieval is running on.': 'Le serveur n’a pas encore dit sur quoi repose la recherche.',
  '{n} vector is waiting for a rebuild.': '{n} vecteur attend d’être reconstruit.',
  '{n} vectors are waiting for a rebuild.': '{n} vecteurs attendent d’être reconstruits.',
  'Vectors pending': 'Vecteurs en attente',
  'Findable by its words already; its vectors are being computed in the background.': 'Déjà trouvable par ses mots ; ses vecteurs sont calculés en arrière-plan.',
  '{n} passages indexed; their vectors are being computed in the background.': '{n} passages indexés ; leurs vecteurs sont calculés en arrière-plan.',
  Embeddings: 'Embeddings',
  '`local` loads the sentence-transformer shipped with the image and switches search to meaning — in French and English — once it is ready; until then, and under `hash`, search matches words. Every stored vector is rebuilt in the background after a switch, and the Memory page says how many still wait.': '`local` charge le sentence-transformer livré avec l’image et fait passer la recherche au sens — en français comme en anglais — dès qu’il est prêt ; d’ici là, et sous `hash`, la recherche compare les mots. Chaque vecteur stocké est reconstruit en arrière-plan après un changement, et la page Mémoire dit combien attendent encore.',
  'Say something.': 'Dites quelque chose.',
  'Ask Metaclaude': 'Demander à Metaclaude',
  'Ask about this deployment, or hand over something to do…': 'Posez une question sur ce déploiement, ou confiez-lui quelque chose à faire…',
  'It reads everything, changes what is reversible, and asks before anything irreversible.': 'Il lit tout, change ce qui est réversible, et demande avant toute action irréversible.',
  'The system workspace is not ready. See the server log.': 'Le workspace système n’est pas prêt. Voir le journal du serveur.',
  ...
  'Kept': 'Retenus',
  'Rejected': 'Rejetés',
  'Insights you kept': 'Enseignements retenus',
  'Insights you rejected': 'Enseignements rejetés',
  'Forget {title}': 'Oublier {title}',
  'Model: {value}': 'Modèle : {value}',
  'Effort: {value}': 'Effort : {value}',
  'How hard it thinks before answering': 'À quel point il réfléchit avant de répondre',
  'What this deployment is running on.': 'Ce sur quoi tourne ce déploiement.',
  'What the scheduled run is given': 'Ce que reçoit le run planifié',
  'Server': 'Serveur',
  ...
  Durable: 'Durable',
  Volatile: 'Volatile',
  'Filter by shelf': 'Filtrer par étagère',
  'Every shelf': 'Toutes les étagères',
  'Shelf': 'Étagère',
  'Standing — a convention, injected into every run of its scope': 'Convention — injectée dans chaque run de sa portée',
  'Durable — recalled by relevance, forgotten slowly': 'Durable — rappelée par pertinence, oubliée lentement',
  'Volatile — a fact that can stop being true, forgotten three times faster': 'Volatile — un fait qui peut cesser d’être vrai, oublié trois fois plus vite',
  'How long this is meant to hold. A convention applies whatever the request is about.': 'Combien de temps ceci doit tenir. Une convention s’applique quel que soit le sujet de la demande.',
  Retire: 'Retirer',
  'Leaves recall at once; restorable for thirty days': 'Sort du rappel immédiatement ; restaurable pendant trente jours',
  'Memory retired': 'Mémoire retirée',
  ...
  'Could not rename the session.': 'Impossible de renommer la session.',
  'Rename session': 'Renommer la session',
  Rename: 'Renommer',
  'What this session is about, in a few words. It appears in the sidebar and nowhere else.':
    'Ce dont parle cette session, en quelques mots. Le titre n’apparaît que dans la barre latérale.',
  'Unread reply': 'Réponse non lue',
  '{n} session with an unread reply': '{n} session avec une réponse non lue',
  '{n} sessions with an unread reply': '{n} sessions avec une réponse non lue',
  '{shown} shown': '{shown} affichées',
  '{shown} shown of {total}': '{shown} affichées sur {total}',
  'Retired memory ({n})': 'Mémoire retirée ({n})',
  'Retired memories ({n})': 'Mémoires retirées ({n})',
  'Collected thirty days after retirement. Restore one to bring it back into recall.': 'Collectées trente jours après le retrait. Restaurez-en une pour la remettre en rappel.',
  'replaced by a newer memory': 'remplacée par une mémoire plus récente',
  Keep: 'Garder',
  'Kept as a memory': 'Gardée en mémoire',
  'Could not keep that note.': 'Impossible de garder cette note.',
  superseded: 'remplace',
  skipped: 'écartée',
  ...
  'What the memory gate made of each note this run proposed. A refused note can still be kept, and a kept one forgotten.':
    'Ce que le filtre de mémoire a fait de chaque note proposée par ce run. Une note refusée peut être conservée, et une note conservée oubliée.',
  'standing': 'convention',
  'volatile': 'volatile',
  durable: 'durable',
  'Restore {title}': 'Restaurer {title}',
  'Keep {title}': 'Garder {title}',
  ...
  'This deployment has no public address configured (METACLAUDE_PUBLIC_URL), so the endpoint cannot be shown here. Tokens still work — the address is your site’s, followed by /api/gateway/mcp.': 'Ce déploiement n’a pas d’adresse publique configurée (METACLAUDE_PUBLIC_URL) : l’adresse ne peut pas être affichée ici. Les jetons fonctionnent quand même — c’est celle de votre site, suivie de /api/gateway/mcp.',
  'No tokens yet': 'Aucun jeton',
  'Nothing outside this deployment can reach the agent.': 'Rien d’extérieur à ce déploiement n’atteint l’agent.',
  'revoked': 'révoqué',
  'expired': 'expiré',
  'can start runs': 'peut lancer des runs',
  'read only': 'lecture seule',
  'expires {when}': 'expire le {when}',
  'expired {when}': 'expiré le {when}',
  'last used {when}': 'dernier usage {when}',
  'Revoke {name}': 'Révoquer {name}',
  'New MCP token': 'Nouveau jeton MCP',
  'The value is shown once, when it is created.': 'La valeur n’est affichée qu’une fois, à la création.',
  'Create token': 'Créer le jeton',
  'The application that will use it': 'L’application qui l’utilisera',
  'Workspaces it can reach': 'Espaces qu’il peut atteindre',
  ...
  'Run what is already allowed': 'Exécuter ce qui est déjà autorisé',
  'Run and edit files': 'Exécuter et modifier les fichiers',
  'Expires in (days)': 'Expire dans (jours)',
  'Copy your token now': 'Copiez votre jeton maintenant',
  'This is the only time it is shown. Paste it into the other application as an Authorization: Bearer header.': 'C’est la seule fois où il est affiché. Collez-le dans l’autre application comme en-tête Authorization: Bearer.',
  'I have saved it': 'Je l’ai enregistré',
  'Copy the token': 'Copier le jeton',
  'Treat it like a password: anything holding it can ask this agent to work in the workspaces you named.': 'Traitez-le comme un mot de passe : tout ce qui le détient peut demander à cet agent de travailler dans les espaces que vous avez nommés.',
  'Revoke this token?': 'Révoquer ce jeton ?',
  'Anything using it stops working immediately. This cannot be undone — issue a new token instead.': 'Tout ce qui l’utilise cesse de fonctionner immédiatement. C’est irréversible — émettez plutôt un nouveau jeton.',
  'Revoke': 'Révoquer',
  'started by an automation': 'lancé par une automatisation',
  'started by a loop': 'lancé par une boucle',
  'started by the system': 'lancé par le système',
  'asked by another workspace': 'demandé par un autre espace',
  'asked through the API': 'demandé via l’API',
  'models': 'les modèles',
  'slash commands': 'les commandes slash',
  'subagents': 'les sous-agents',
  'account details': 'le compte',
  'plugin': 'plugin',
  'Ops': 'Ops',
  'Not connected': 'Non connecté',

  /* ---- The knowledge library: files, reach and provenance ---- */
  'Larger than {size}.': 'Plus lourd que {size}.',
  'This type is not read. Accepted: {list}.': 'Ce type n’est pas lu. Acceptés : {list}.',
  'Could not upload that file.': 'Impossible de déposer ce fichier.',
  'Drop files here, or': 'Déposez des fichiers ici, ou',
  'choose them': 'choisissez-les',
  max: 'max',
  'Choose files to add to the library': 'Choisir des fichiers à ajouter à la bibliothèque',
  Uploads: 'Dépôts',
  'Uploading…': 'Envoi…',
  'Clear the list': 'Vider la liste',
  '{n} passage indexed': '{n} passage indexé',
  '{n} passages indexed': '{n} passages indexés',
  '{n} passage': '{n} passage',
  '{n} page': '{n} page',
  '{n} pages': '{n} pages',
  ...
  '{n} hidden': '{n} masqué(s)',
  'Read again with {engine}': 'Relu avec {engine}',
  'Could not re-extract that file.': 'Impossible de relire ce fichier.',
  'Read the file again': 'Relire le fichier',
  'Download the original': 'Télécharger l’original',
  Document: 'Document',
  'This document could not be opened.': 'Ce document n’a pas pu être ouvert.',
  'Reference documents the agent can quote — a lease, a spec, a runbook. Drop a file or paste text, then choose which workspaces it reaches. Runs retrieve the relevant passages automatically and cite them by page and line.': 'Des documents de référence que l’agent peut citer — un bail, une spec, un runbook. Déposez un fichier ou collez du texte, puis choisissez les espaces de travail qu’il atteint. Les runs récupèrent automatiquement les passages pertinents et les citent par page et par ligne.',
  'Paste a document': 'Coller un document',
  'Filter by name': 'Filtrer par nom',
  'Find a document': 'Trouver un document',
  'No document matches': 'Aucun document ne correspond',
  'Drop the documents your runs keep needing — the contract, the conventions, the runbook — and the agent will cite them instead of guessing.': 'Déposez les documents dont vos runs ont sans cesse besoin — le contrat, les conventions, le runbook — et l’agent les citera au lieu de deviner.',
  'Try another word, or widen the workspace filter.': 'Essayez un autre mot, ou élargissez le filtre d’espace de travail.',
  'More actions for “{name}”': 'Autres actions pour « {name} »',
  View: 'Voir',
  'Rename and re-aim': 'Renommer et réorienter',
  'This document was read from a file, so its text is not edited here — read the file again to pick up an extractor improvement.': 'Ce document a été lu depuis un fichier : son texte ne se modifie pas ici — relisez le fichier pour profiter d’une amélioration de l’extraction.',
  'and every passage indexed from it are removed, along with the original file. Runs stop seeing it immediately.': 'ainsi que tous les passages indexés depuis lui sont supprimés, avec le fichier d’origine. Les runs cessent de le voir immédiatement.',
  '{n} passage indexed; its vectors are being computed in the background.': '{n} passage indexé ; son vecteur est calculé en arrière-plan.',
  '{n} passage indexed and ready to be retrieved.': '{n} passage indexé, prêt à être récupéré.',
  '{n} passage re-embedded.': '{n} passage ré-embarqué.',
  'p.': 'p.',
  'diapo.': 'diapo.',
  feuille: 'feuille',
  feuilles: 'feuilles',
