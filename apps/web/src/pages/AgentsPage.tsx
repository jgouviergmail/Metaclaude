    ...
    <AppShell>
      ...
      <div className="flex-1 overflow-y-auto">
        <Tabs value={tab} onValueChange={(value) => setTab(value as TabKey)}>
          {/*
            * The strip sticks, so it sits outside the scrolling body and needs
            * to be told the width that body uses — otherwise the triggers
            * start at the screen's edge and the content they label a hundred
            * and eighty pixels further in.
            */}
          <TabStrip label={t('Extension type')} width="standard" sticky>
            {(
              [
                { value: 'skills', label: 'Skills', icon: <Sparkles /> },
                { value: 'agents', label: 'Subagents', icon: <Bot /> },
                { value: 'mcp', label: 'MCP servers', icon: <Plug /> },
                ...
                { value: 'claude', label: t('From Claude'), icon: <Wand2 /> },
              ] as const
            ).map((entry) => (
              <TabTrigger key={entry.value} value={entry.value} icon={entry.icon}>
                {t(entry.label)}
              </TabTrigger>
            ))}
          </TabStrip>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <code className="font-mono text-body font-medium text-ink">{skill.name}</code>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {nameError ? (
          <p id="skill-name-error" role="alert" className="-mt-2 text-caption leading-relaxed text-danger">
            {nameError}
          </p>
        ) : null}
          ...
          {agents.map((agent) => (
            <Card key={agent.id} className="p-4">
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                <div className="min-w-0 flex-1 space-y-1.5">
                  <div className="flex flex-wrap items-center gap-2">
                    <code className="font-mono text-body font-medium text-ink">{agent.name}</code>
                  ...
                  <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-caption text-subtle">
                    <span>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {nameError ? (
          <p id="agent-name-error" role="alert" className="-mt-2 text-caption leading-relaxed text-danger">
            {nameError}
          </p>
        ) : null}
  ...
  return (
    <Card className="p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
        <div className="min-w-0 flex-1 space-y-1.5">
          <div className="flex flex-wrap items-center gap-2">
            <code className="font-mono text-body font-medium text-ink">{entry.name}</code>
            <Badge tone="thinking">{entry.kind === 'agent' ? t('subagent') : t('skill')}</Badge>
            <Badge tone="info">{t(CATEGORY_LABELS[entry.category])}</Badge>
          </div>
          <p className="text-body leading-relaxed text-muted">{entry.description}</p>
        </div>
              ...
              <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
                <div className="min-w-0 flex-1 space-y-1.5">
                  ...
                  {server.envKeys.length > 0 ? (
                    <p className="flex flex-wrap items-center gap-1 text-caption text-subtle">
                  ...
                  {live.get(server.name)?.error ?? server.lastError ? (
                    <p className="rounded-lg border border-danger/25 bg-danger-soft px-2.5 py-1.5 text-caption leading-relaxed text-danger">
                      {live.get(server.name)?.error ?? server.lastError}
                    </p>
                  ) : null}
                  ...
                  {describedFor(server)?.instructions ? (
                    ...
                    <details className="group rounded-lg border border-line bg-sunken/40">
                      <summary className="flex cursor-pointer list-none items-center gap-1.5 rounded-lg px-2.5 py-1.5 text-caption text-muted hover:text-ink focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-accent">
                        <ChevronRight
                      ...
                      <p className="border-t border-line px-2.5 py-2 text-caption leading-relaxed text-muted">
                        {describedFor(server)?.instructions}
                      </p>
                    </details>
                  ) : null}
...
function ConnectorDirectory({ onInstalled }: { onInstalled: () => void }) {
  ...
  return (
    /*
     * A `Section`, not a hand-written heading over a paragraph.
     *
     * Five sentences, always shown, filled a phone's remaining screen on their
     * own — and the heading was an `h3` directly under the page's `h1`, so the
     * outline a screen reader reads announced a subsection of nothing. Both are
    ...
    <Card className="p-4">
      <div className="space-y-3">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start">
          <div className="min-w-0 flex-1 space-y-1.5">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-body font-medium text-ink">{connector.title}</span>
            ...
            <p className="text-body leading-relaxed text-muted">{connector.description}</p>
            <p className="break-all font-mono text-caption leading-relaxed text-subtle">
        ...
        {credential && !connector.installed && !armed ? (
          <p className="flex flex-wrap items-center gap-1.5 text-caption text-subtle">
            ...
            <code className="font-mono text-caption text-muted">{credential.key}</code>
          </p>
        ) : null}
          ...
          <div className="space-y-1.5 rounded-lg border border-line bg-sunken/40 p-3">
            <Label htmlFor={fieldId}>
              <span className="inline-flex items-center gap-1.5">
                <KeyRound className="size-3.5" aria-hidden />
                <code className="font-mono text-caption">{credential.key}</code>
                {credential.required ? null : (
                  <span className="text-caption font-normal text-subtle">{t('optional')}</span>
                )}
              </span>
            </Label>
            {/* The hint sits outside the label on purpose: aria-describedby
                describes, it does not remove text from the accessible name. */}
            <p id={`${fieldId}-hint`} className="text-caption leading-relaxed text-muted">
              {credential.hint}
            </p>
            <Input
              id={fieldId}
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        {nameError ? (
          <p id="mcp-name-error" role="alert" className="-mt-2 text-caption leading-relaxed text-danger">
            {nameError}
          </p>
        ) : null}
...
function SectionIntro({
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-3">
      <p className="max-w-2xl text-body leading-relaxed text-muted">{description}</p>
...
function PairEditor({
  ...
  return (
    <fieldset className="space-y-2">
      ...
      {hint ? <p className="text-caption leading-relaxed text-muted">{hint}</p> : null}

      {pairs.length === 0 ? (
        <p className="text-caption text-subtle">{t('None.')}</p>
      ) : (
          ...
          {pairs.map((pair, index) => (
            <li key={index} className="flex flex-col gap-2 sm:flex-row sm:items-center">
              <Input
                ...
                className="font-mono text-body sm:flex-1"
              />
              <Input
                value={pair.value}
