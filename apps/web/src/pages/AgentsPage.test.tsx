...
/**
 * The availability filter, wired into a tab.
 *
 * `AvailabilityFilter.test.tsx` covers the chips themselves; what only this
 * level can see is that the tab narrows the list *and* narrows what the bulk
 * buttons act on. `BulkActions` promises to act on "the rows the operator is
 * looking at", so handing it the unfiltered list would make "disable all"
 * reach past the screen — silently, and destructively for "delete all".
 */
describe('the availability filter on the skills tab', () => {
  const skill = (name: string, enabled: boolean) => ({
    id: `skill_${name}`,
    workspaceId: null,
    name,
    description: `${name} fait quelque chose.`,
    body: '# corps',
    category: 'general' as const,
    enabled,
    autoGenerated: false,
  ...
  beforeEach(() => {
    apiMock.skills.mockResolvedValue({
      skills: [skill('revue-sql', true), skill('notes-de-version', false)],
    } as never);
  });

  it('shows everything until asked otherwise', async () => {
    renderWithProviders(<AgentsPage />);
    expect(await screen.findByText('revue-sql')).toBeDefined();
    expect(screen.getByText('notes-de-version')).toBeDefined();
  });

  it('narrows to the inactive ones, and back', async () => {
    renderWithProviders(<AgentsPage />);
    await screen.findByText('revue-sql');

    const group = screen.getByRole('group', { name: 'Skill availability' });
    fireEvent.click(within(group).getByRole('button', { name: /Inactive/ }));

    expect(screen.queryByText('revue-sql')).toBeNull();
    expect(screen.getByText('notes-de-version')).toBeDefined();

    fireEvent.click(within(group).getByRole('button', { name: /All statuses/ }));
    expect(screen.getByText('revue-sql')).toBeDefined();
  });

  it('says which emptiness it is when the filter matches nothing', async () => {
    apiMock.skills.mockResolvedValue({ skills: [skill('revue-sql', true)] } as never);
    renderWithProviders(<AgentsPage />);
    await screen.findByText('revue-sql');
    ...
    // "No skills in this scope" would be a lie: there is one, it is just on.
    expect(screen.getByText('Nothing in this state')).toBeDefined();
    expect(screen.queryByText('No skills in this scope')).toBeNull();
  });

  it('keeps the bulk buttons pointed at the rows on screen', async () => {
    renderWithProviders(<AgentsPage />);
    await screen.findByText('revue-sql');
    ...
    // One row shown, and it is already disabled — so "disable all" has nothing
    // left to do. With the unfiltered list it would still offer the enabled one.
    expect((screen.getByRole('button', { name: 'Disable all' }) as HTMLButtonElement).disabled)
      .toBe(true);

    // And the other way round, so neither result is an accident of which
    // button happens to be greyed out.
    fireEvent.click(within(group).getByRole('button', { name: /Active/ }));
    expect((screen.getByRole('button', { name: 'Enable all' }) as HTMLButtonElement).disabled)
      .toBe(true);
    expect((screen.getByRole('button', { name: 'Disable all' }) as HTMLButtonElement).disabled)
      .toBe(false);
  });
